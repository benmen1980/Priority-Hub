...
var APIVersion = '2.3.13', AppMain, BuildVersion = 0, HasFiltersFeature, HasFormAlertChooseFeature, IPMessage = '', LangFormat = 0, MGCookie = '', MajorVersion = 0, MinorVersion = 0, NotSupportedFunctions, SystemImagesDir = '', SystemMailDir = '', TenantID = '', isDebugging = true, logger_0, tenYearsFromNow, timeZoneName = '@@@@@@';
...
function $FormFieldUpdate(this$static, fieldName, value_0, onSuccess, onError, executeNow){
  ...
  valueSave = value_0;
  col.type_0 == 'date' && (value_0 = ISOToDate(col.width_0, value_0, true));
  if (value_0 == 'NOTISO') {
...
function ISOToDate(colWidth, ISODate, fromExternal){
  $clinit_Form();
  if (ISODate == null || ISODate == '')
    return '';
  if (!/\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d(\.\d+)?([+-][0-2]\d:[0-5]\d|Z)/.test(ISODate) && !/\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d(\.\d+)?/.test(ISODate) && !/\d{4}-[01]\d-[0-3]\d/.test(ISODate))
    return ISODate;
  if (colWidth >= 14 || fromExternal) {
    ...
    for (var i = 0; i < jo.Form[0].NewTable.length; i++) {
      ...
      if (maintable.NewColumn && maintable.NewColumn.length > 0) {
        for (var j = 0; j < maintable.NewColumn.length; j++) {
          ...
          var readonly = maintable.NewColumn[j].readonly == 'r'?1:0;
          var isstatus = 0;
...
function $CurFieldVal(this$static){
  ...
  if (!!fc && fc.type_0 == 'date')
    return ISOToDate(fc.width_0, this$static.curfieldval, false);
...
function FixFilterValue(value_0, col){
  var dtval = value_0;
  col.type == 'date' && (dtval = ISOToDate(col.maxLength, value_0, true));
  return dtval;
}
  ...
  for (i = 0; i < editFields.jsArray.length; i++) {
    ...
    if (!!fi && $equals('date', fi.columntype.inputType)) {
      if ('value' in jv.jsObject) {
        ...
        if (s) {
          datestr = ISOToDate(fi.width_0, s.value_0, true);
